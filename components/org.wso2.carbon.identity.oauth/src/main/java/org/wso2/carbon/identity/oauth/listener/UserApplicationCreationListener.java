...
public class UserApplicationCreationListener extends AbstractIdentityUserOperationEventListener {
    ...
    public boolean doPostAddUserWithID(User user, Object credential, String[] roleList,
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Agent detected: " + username + " in domain: " + userStoreDomain +
                        ". Creating standard-based OAuth2/OIDC application.");
            }

            // Create the OAuth2/OIDC application for the agent
            createStandardBasedApplication(username, userId, tenantDomain);

            if (log.isDebugEnabled()) {
                log.debug("Successfully created standard-based OAuth2/OIDC application for agent: " +
                        username);
            }
            ...
            log.error("Unexpected error occurred while creating standard-based application for agent: "
                    +
                    user.getUsername(), e);
            // Return true to not block agent creation
            return true;
        }
    }

    /**
     * Creates a standard-based application for the agent following the exact pattern used by the
     framework.
     * This method uses ApplicationDTO and calls protocol handlers to configure OAuth2/OIDC inbound
     * authentication. The application name (application ID) will be set to the agent username
     * (without domain prefix).
     *
     * @param username     Username of the newly created agent (includes domain prefix e.g.,
    AGENT/agentname)
     * @param userId       User ID of the newly created agent
     * @param tenantDomain Tenant domain
     * @throws IdentityApplicationManagementException if application creation fails
     */
    private void createStandardBasedApplication(String username, String userId, String tenantDomain)
            throws IdentityApplicationManagementException {

        // Extract the agent name without domain prefix (e.g., "AGENT/agent123" -> "agent123")
        String agentName = UserCoreUtil.removeDomainFromName(username);

        // Application name = agent username (as per requirement: application ID should equal agent
        //username)
        String applicationName = agentName;
        ...
        // Create InboundProtocolsDTO (empty - handlers will be invoked by framework)
        // When InboundProtocolsDTO is null or empty, the framework will use default configurations
        // or rely on templates for standard-based applications

        OAuthConsumerAppDTO consumerAppDTO = new OAuthConsumerAppDTO();
        consumerAppDTO.setApplicationName(applicationName);
        ...
        // Create the standard-based application using ApplicationDTO
        // This follows the exact flow: createApplication(ApplicationDTO) ->
        // addInboundAuthenticationProtocolsToApplication() -> createApplication(ServiceProvider)
        // The framework will invoke protocol handlers and listeners properly
        String applicationResourceId = applicationManagementService.createApplication(
                applicationDTO, tenantDomain, username);

        if (log.isDebugEnabled()) {
            log.debug("Successfully created standard-based application with name: " + applicationName +
                    " (Resource ID: " + applicationResourceId + ") for agent: " + username +
